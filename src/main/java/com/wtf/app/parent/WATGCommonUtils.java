...
import com.wtf.app.model.enums.ActionIfTheGroupIsEligible;
import com.wtf.app.model.enums.EnumStringToExport;
...
public abstract class WATGCommonUtils extends SocialParentCommonUtils {
    ...
    private static final long DEFAULT_RETRY_INTERVAL_MS = 10_000; // Reduced from 30s to 10s
    private static final int DEFAULT_MAX_RETRIES = 2; // Reduced from 3 to 2
    private static final long DEFAULT_MAX_TOTAL_RETRY_TIME_MS = 300000; // 5 minutes
    private static int SLEEP_TIME_MS = 500;//2000;
        ...
        while (retryCount < maxRetries) {
            ...
            try {
                ...
                WebElement elementScanner = RetryUtils.withRetry(
                        ...
                        3,      // max retries
                        200     // delay between retries in ms 5k
                );
    ...
    protected void runActionClassForNextChat() {
        ...
        logger.info("runActionClassForNextChat: Action class Executed SLEEP_TIME_MS :" + SLEEP_TIME_MS);
    ...
    void runActionClassMoveToChat(boolean moveToPrevOnly, int SLEEP_TIME_MS) {
        try {
            logger.info("runActionClassMoveToChat: Action class Executed SLEEP_TIME_MS :" + SLEEP_TIME_MS);
            ...
            logger.info("isExecutionForWhatsApp MoveToChat: " + isExecutionForWhatsApp + "  moveToPrevOnly" + moveToPrevOnly);
            //Key Down
            if (isExecutionForWhatsApp) {
            ...
            if (moveToPrevOnly) {
                ...
                actions.keyDown(xPathInterfaceWATG.getMoveToPreviousChatKeys());//send keys don't need perform
                logger.info("moveToPrevOnly ::: " + moveToPrevOnly);
            } else {
                actions.keyDown(xPathInterfaceWATG.getMoveToNextChatKeys());
                logger.info("nextChatExecuted....");
            }
            Thread.sleep(200);
            ...
            //Release keys
            //Key UP
            actions.keyUp(xPathInterfaceWATG.getMoveToChatKeysAlt());
            if (isExecutionForWhatsApp) {
            ...
            if (moveToPrevOnly) {
                ...
                actions.keyUp(xPathInterfaceWATG.getMoveToPreviousChatKeys());//send keys don't need perform
                logger.info("moveToPrevOnly ::: " + moveToPrevOnly);
            } else {
                actions.keyUp(xPathInterfaceWATG.getMoveToNextChatKeys());
                logger.info("nextChatExecuted....");
            }
            Thread.sleep(200);
            actions.build().perform();
            ...
            logger.info("Next/Previous Chat Executed Successfully");
    ...
    public boolean searchAndClickOnGroup(String oldGroupString) {
        ...
        try {
            ...
            try {
                ...
                if (!isClickedSuccessful)
                    isClickedSuccessful = febxsAndClickWithRetry("//span[@data-icon='back']/parent::div", "//span[@data-icon='search']", false, "BACK_ARROW__BESIDE_MAGNIFYING_GLASS_SEARCH_TRYING_OTHERS");
